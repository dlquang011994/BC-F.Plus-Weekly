...
def build(input_path: Path) -> dict:
    ...
    for row in sales_rows:
        ...
        # Giữ đúng dự phòng của dashboard gốc khi cột Phân loại không có tên.
        if not classification and len(row) >= 4:
            classification = row_value(row, 3)
        # Vùng trên phiếu bán là ưu tiên; Danhba chỉ là dự phòng khi ô này trống.
    ...
    for shop, classification, raw_region, week, month in sales_data:
        if not shop or classification.lower() != "lấy":
            continue
        key = shop.lower()
        region = raw_region or shop_to_region.get(key, "Khác")
        if not region or region.lower() == "loại":
            continue
        all_regions.add(region)
        if month in months_to_show:
